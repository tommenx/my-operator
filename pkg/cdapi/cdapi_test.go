...
func TestCdClient_SetOneLimit(t *testing.T) {
	cd := NewCDClient("http://10.77.110.131:8888", time.Second*5)
	instances := []*Instance{
		{"tidb-cluster-tikv-0", "10", "20"},
		{"tidb-cluster-tikv-1", "20", "30"},
	}
...
func TestCdClient_SetBatchLimit(t *testing.T) {
	cd := NewCDClient("http://10.77.110.131:8888", time.Second*5)
	err := cd.SetBatchLimit("default", "app.kubernetes.io/component", "tikv", "25", "25")
	if err != nil {
		t.Errorf("set batch limit error, err=%+v", err)
	}
}
